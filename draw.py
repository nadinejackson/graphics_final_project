...
def draw_gscanline(x0, z0, x1, z1, y, screen, zbuffer, c0, c1):
    ...
    ctr = c0[:]
    for i in range(3):
        delta_c[i] = (c1[i] - c0[i]) / (x1 - x0 + 1) if (x1 - x0 + 1) != 0 else 0
    while x <= x1:
        plot(screen, zbuffer, [int(ctr[0]), int(ctr[1]), int(ctr[2])], x, y, z)
        x+= 1
        z+= delta_z
        for i in range(3):
            ctr[i] += delta_c[i]
...
def iterate_convert(polygons, i, screen, zbuffer, normals, shading,  view, ambient, light, symbols, reflect):
    ...
    normal = normals[(int(points[BOT][0] * 100) / 100.0,\
...
def add_mesh(polygons, filename):
    ...
    for line in g:
        if "v " == line[0:2]:
            ...
            for point in face:
                if "/" in point:
                    triangle += verticies[int(point.split("/")[0])]
                else:
                    triangle += verticies[int(point)]
            if len(face) == 3:
                add_polygon(polygons, triangle[0], triangle[1], triangle[2],\
                ...
                add_polygon(polygons, triangle[3], triangle[4], triangle[5],\
                            triangle[6], triangle[7], triangle[8],\
                            triangle[9], triangle[10], triangle[11])
            else:
...
##                if "/" in word:
##                    coords.append(float(word.split("/")[0]))
